...
bool kernelmode_proc_handler::attach(const char* proc_name) {
	...
	if (!is_admin) {
		printf(_xor_("Run as admin!").c_str());
		return false;
	}
	...
	handle = CreateFileA(_xor_("\\\\.\\esa204").c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0);

	if (handle == INVALID_HANDLE_VALUE) {
		printf(_xor_("No driver loaded!").c_str());
		return false;
	}
...
uint32_t kernelmode_proc_handler::virtual_protect(uint64_t address, size_t size, uint32_t protect) {
	...
	if (DeviceIoControl(handle, ioctl_protect_virtual_memory, &request, sizeof(k_protect_mem_request), &request, sizeof(k_protect_mem_request), &bytes_read, 0))
		return protect;
